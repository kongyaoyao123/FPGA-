...
// feature map buffers
dtype_FM FM_buf1[32][44][84];
dtype_FM FM_buf2[32][44][84];
dtype_FM FM_buf3[32][44][84];
dtype_FM FM_buf4[32][44][84];
dtype_FM_acc FM_buf_acc[32][44][84];

// weight buffers
dtype_WT weight_buf_1x1[4][32][32];
dtype_WT weight_buf_3x3[4][32][3][3];
dtype_WT bias_buf[4][32];



void compute_bounding_box(float predict_box[4][5], int constant[4][3])
{
    dtype_32_4 conf_thresh = -100.0;
    int conf_j = 0;
    int conf_m = 0;
    int conf_n = 0;
    dtype_32_4 conf_box1 = 0.0;
    dtype_32_4 conf_box2 = 0.0;
    //int h = 20;
    //int w = 40;
    ...
    for(int m = 1; m <= 20; m++){
        for(int n = 1; n <= 40; n++){
            //conf_box1 = (dtype_32_25)1 / ((dtype_32_25)1 + my_exp_fix(-FM_buf_acc[4][m][n]));
    ...
    for(int m = 1; m <= 20; m++){
        for(int n = 1; n <= 40; n++){
            //conf_box2 = (dtype_32_25)1 / ((dtype_32_25)1 + my_exp_fix(-FM_buf_acc[9][m][n]));
...
inline dtype_FM relu_single( dtype_FM d ) {
	if( d > 6 )
		return 6;
	if( d < 0 )
		return 0;
...
inline dtype_FM_acc fmrg_acc(uint256 DATA, int offset){
	dtype_FM_acc d = 0;
	d.range(FM_RG,0) = DATA.range(offset + FM_RG, offset);
	return d;
}

inline dtype_FM fmrg(uint256 DATA, int offset){
	dtype_FM d = 0;
	d.range(FM_RG,0) = DATA.range(offset + FM_RG, offset);
	return d;
}
...
void relu_copy_buf_to_DDR( uint256* dest, int buf_id, dtype_FM src[32][44][84], int offset_h, int offset_w)
{
...
#pragma HLS pipeline
			...
			for(int c = 0; c < 32; c++) {
#pragma HLS unroll
				dtype_FM d = relu_single(src[c][h][w]);
...
void relu_copy_buf_to_DDR_acc( uint256* dest, int buf_id, dtype_FM_acc src[32][44][84], int offset_h, int offset_w)
{
	uint256* dest_ptr = dest + 44*84*buf_id + (offset_h*22)*84 + (offset_w*42);
...
#pragma HLS pipeline
			...
			for(int c = 0; c < 32; c++) {
#pragma HLS unroll
				dtype_FM d = relu_single((dtype_FM)src[c][h][w]);
				DATA.range(FM_RG + 8*c, 8*c) = d.range(FM_RG, 0);
			}
			dest_ptr[w].range(255, 0) = DATA.range(255, 0);
		}
...
void load_buf_from_DDR( dtype_FM dest[32][44][84], uint256* src, int buf_id)
{
...
void load_weight_1x1_from_axi( dtype_WT dest[32][32], uint512 src[32])
{
...
void load_weight_3x3_from_axi( dtype_WT dest[32][3][3], uint512 src[3][3])
{
	for(int m = 0; m < 3; m++) {
		for(int n = 0; n < 3; n++) {
#pragma HLS pipeline
...
void load_bias_from_axi(dtype_WT dest[32], uint512 src)
{
	for(int c = 0; c < 32; c++) {
#pragma HLS unroll
...
void set_bias_1x1( dtype_FM_acc buf[32][44][84], dtype_WT bias[32])
{
...
void set_bias_3x3( dtype_FM buf[32][44][84], dtype_WT bias[32])
{
#pragma HLS array_partition variable=bias dim=1 complete
...
void Relu_Convert_dtype_set_bias_3x3( dtype_FM_acc buf_acc[32][44][84], dtype_FM buf_bottom[32][44][84],
							   dtype_FM buf_top[32][44][84], dtype_WT bias[32])
{
#pragma HLS array_partition variable=bias dim=1 complete
...
#pragma HLS pipeline
			for(int c = 0; c < 32; c++) {
#pragma HLS unroll
				buf_bottom[c][h][w] = relu_single((dtype_FM)buf_acc[c][h][w]);
...
void Relu_Convert_FIX( dtype_FM_acc buf_acc[32][44][84], dtype_FM buf_bottom[32][44][84])
{
// #pragma HLS array_partition variable=bias dim=1 complete
#pragma HLS array_partition variable=buf_acc dim=1 complete
#pragma HLS array_partition variable=buf_bottom dim=1 complete
...
void clear_buffer( dtype_FM_acc buf[32][44][84] )
{
...
void load_image_chunk_norm(dtype_FM img_buf[32][44][84], int bindata_in_burst[3*162*2*324*2*2],
							int col, int row, int offset_h = 0, int offset_w = 0)
{
	int* bindata_in_burst_ptr;

	bindata_in_burst_ptr = bindata_in_burst + (col*40 + offset_h*2)*322*2 + row*80 + offset_w*2;
	for(int i = 0; i < 44; i++) {
		for(int j = 0; j < 84; j++) {
#pragma HLS pipeline
				img_buf[0][i][j] = img_norm_ch[((bindata_in_burst_ptr[j]))];
		}
		bindata_in_burst_ptr += 322*2;
	}

	bindata_in_burst_ptr = bindata_in_burst + 1*162*2*322*2 + (col*40 + offset_h*2)*322*2 + row*80 + offset_w*2;
	for(int i = 0; i < 44; i++) {
		for(int j = 0; j < 84; j++) {
#pragma HLS pipeline
				img_buf[1][i][j] = img_norm_ch[((bindata_in_burst_ptr[j]))];
		}
		bindata_in_burst_ptr += 322*2;
	}

	bindata_in_burst_ptr = bindata_in_burst + 2*162*2*322*2 + (col*40 + offset_h*2)*322*2 + row*80 + offset_w*2;
	for(int i = 0; i < 44; i++) {
		for(int j = 0; j < 84; j++) {
#pragma HLS pipeline
				img_buf[2][i][j] = img_norm_ch[((bindata_in_burst_ptr[j]))];
		}
		bindata_in_burst_ptr += 322*2;
	}
}
...
void Relu6_3x3( dtype_FM buf[32][44][84])
{
	for(int h = 1; h <= 42; h+=2) {
		for(int w = 1; w <= 82; w++) {
#pragma HLS pipeline
...
inline dtype_FM relu_max(dtype_FM a, dtype_FM b, dtype_FM c, dtype_FM d)
{
	dtype_FM t1, t2;
...
#pragma HLS pipeline II=1
            ...
            for(int c = 0; c < 32; c++) {
#pragma HLS unroll
                dtype_FM d = relu_max(buf_in[c][h*2-1][w*2-1], buf_in[c][h*2-1][w*2],
...
void load_and_reorg_part( uint256* buf_in, int buf_id,
						  dtype_FM buf_out_1[32][44][84],
						  dtype_FM buf_out_2[32][44][84],
						  dtype_FM buf_out_3[32][44][84],
						  dtype_FM_acc buf_out_4[32][44][84],	// borrow one
...
void load_and_reorg( uint256* buf_1, int buf_id_1, uint256* buf_2, int buf_id_2,
					 ...
					 dtype_FM buf_out_2[32][44][84],
					 dtype_FM buf_out_3[32][44][84],
					 dtype_FM_acc buf_out_4[32][44][84])
{
...
void local_buf_copy( dtype_FM dest[32][44][84], dtype_FM_acc src[32][44][84])
{
	for(int h = 0; h < 44; h+=2) {
		for(int w = 0; w < 84; w++) {
#pragma HLS pipeline
...
void buffer_copy_debug( float dest[32][44][84], dtype_FM src[32][44][84])
{
	for(int h = 0; h < 44; h++) {
		for(int w = 0; w < 84; w++) {
			for(int c = 0; c < 32; c++) {
...
void buffer_copy_debug_acc( float dest[32][44][84], dtype_FM_acc src[32][44][84])
{
	for(int h = 0; h < 44; h++) {
		for(int w = 0; w < 84; w++) {
			for(int c = 0; c < 32; c++) {
...
#pragma HLS INTERFACE m_axi depth=3*162*324		port=bindata_in			offset=slave	bundle=IMG

#pragma HLS INTERFACE m_axi depth=306*32*32		port=wright1x1		offset=slave	bundle=BUS512
#pragma HLS INTERFACE m_axi depth=24*32*3*3		port=wright3x3		offset=slave	bundle=BUS512
...
#pragma HLS ALLOCATION instances=convolution3x3	    		limit=1 function
...
#pragma HLS ALLOCATION instances=load_image_chunk_norm		limit=1 function
	...
	load_weight_3x3_from_axi(weight_buf_3x3[0], wright3x3[weight_3x3_index]);
	...
	load_weight_1x1_from_axi(weight_buf_1x1[0], wright1x1[weight_1x1_index + 0]);
	load_weight_1x1_from_axi(weight_buf_1x1[1], wright1x1[weight_1x1_index + 1]);

	for(int row = 0; row < 4; row++) {

		load_image_chunk_norm(FM_buf1, bindata_in, 0, row, 0/4, row/4);
		for(int col = 0; col < 4; col++) {

			if( col % 2 == 0 ) {
				convolution3x3(FM_buf1, FM_buf2, weight_buf_3x3[0], bias_buf[0], 1);
				load_image_chunk_norm(FM_buf3, bindata_in, col+1, row, (col+1)/4, row/4);
			}
			else {
				convolution3x3(FM_buf3, FM_buf2, weight_buf_3x3[0], bias_buf[0], 1);
				load_image_chunk_norm(FM_buf1, bindata_in, col+1, row, (col+1)/4, row/4);
			}

			for(int co = 0; co < CO_N; co++) {
				CONV_1x1_bias(FM_buf2, FM_buf_acc, weight_buf_1x1[co],bias_buf[1 + co],1);
				Relu_Max_Pooling(FM_buf_acc, DDR3buff, 0, co, col, row, col/4, row/4, 1);
			}
		}
	}
	...
	load_weight_3x3_from_axi(weight_buf_3x3[1], wright3x3[weight_3x3_index + 0]);
	load_weight_3x3_from_axi(weight_buf_3x3[2], wright3x3[weight_3x3_index + 1]);
	...
	for(int row = 0; row < 4; row++) {
		for(int col = 0; col < 4; col++) {
			...
			load_dw1_pool_from_DDR(DDR3buff+DW1_POOL_OFFSET, FM_buf1, 0, col, row, col/2, row/2);
			convolution3x3(FM_buf1, FM_buf2, weight_buf_3x3[1 + 0], bias_buf[0 + 0], 1);

			load_dw1_pool_from_DDR(DDR3buff+DW1_POOL_OFFSET, FM_buf4, 1, col, row, col/2, row/2);
			convolution3x3(FM_buf4, FM_buf3, weight_buf_3x3[1 + 1], bias_buf[0 + 1], 1);

			for(int co = 0; co < CO_N; co++) {
				load_bias_from_axi(bias_buf[2], bias_all[bias_1x1_index + co]);

				load_weight_1x1_from_axi(weight_buf_1x1[0], wright1x1[weight_1x1_index + 0 + co * CI_N]);
				CONV_1x1_bias(FM_buf2, FM_buf_acc, weight_buf_1x1[0],bias_buf[2],0,true);

				load_weight_1x1_from_axi(weight_buf_1x1[1], wright1x1[weight_1x1_index + 1 + co * CI_N]);
				CONV_1x1_bias(FM_buf3, FM_buf_acc, weight_buf_1x1[1], bias_buf[2],1,false);

				Relu_Max_Pooling(FM_buf_acc, DDR3buff, 0, co, col, row, col/2, row/2, 2);
			}
		}
	}
	...
	load_weight_3x3_from_axi(weight_buf_3x3[0], wright3x3[weight_3x3_index + 0]);
	load_weight_3x3_from_axi(weight_buf_3x3[1], wright3x3[weight_3x3_index + 1]);
	load_weight_3x3_from_axi(weight_buf_3x3[2], wright3x3[weight_3x3_index + 2]);
	load_bias_from_axi(bias_buf[0], bias_all[bias_3x3_index + 0]);
	load_bias_from_axi(bias_buf[1], bias_all[bias_3x3_index + 1]);
	...
	for(int row = 0; row < 2; row++) {
		for(int col = 0; col < 2; col++) {
			...
			load_dw2_pool_from_DDR(DDR3buff+DW2_POOL_OFFSET, FM_buf1, 0, col, row, col/1, row/1 );
			convolution3x3(FM_buf1, FM_buf2, weight_buf_3x3[0 + 0], bias_buf[0 + 0], 1);

			load_dw2_pool_from_DDR(DDR3buff+DW2_POOL_OFFSET, FM_buf1, 1, col, row, col/1, row/1 );
			convolution3x3(FM_buf1, FM_buf3, weight_buf_3x3[0 + 1], bias_buf[0 + 1], 1);

			load_dw2_pool_from_DDR(DDR3buff+DW2_POOL_OFFSET, FM_buf1, 2, col, row, col/1, row/1 );
			convolution3x3(FM_buf1, FM_buf4, weight_buf_3x3[0 + 2], bias_buf[0 + 2], 1);
			...
			for(int co = 0; co < CO_N; co++) {
				...
				load_weight_1x1_from_axi(weight_buf_1x1[1], wright1x1[weight_1x1_index + 1 + co * CI_N]);
				convolution1x1(FM_buf3, FM_buf_acc, weight_buf_1x1[1], bias_buf[3],0,false);

				load_weight_1x1_from_axi(weight_buf_1x1[2], wright1x1[weight_1x1_index + 2 + co * CI_N]);
				CONV_1x1_bias(FM_buf4, FM_buf_acc, weight_buf_1x1[2], bias_buf[3],0,false);

				relu_copy_buf_to_DDR_acc(DDR3buff+DDR_OFFSET, 100 + co + (col*2 + row) * CO_N, FM_buf_acc, 0, 0);
				Relu_Max_Pooling(FM_buf_acc, DDR3buff, 6 + co, co, col, row, col, row, 3 );
			}
		}
	}
	...
	/// conv3x3: out: FM_buf2
	load_buf_from_DDR(FM_buf1, DDR3buff+DDR_OFFSET, 6 + 0);
	for(int c = 0; c < CI_N; c++) {
		load_weight_3x3_from_axi(weight_buf_3x3[0], wright3x3[weight_3x3_index + c]);
		load_bias_from_axi(bias_buf[0], bias_all[bias_3x3_index + c]);

		if( c % 2 == 0 ) {
			load_buf_from_DDR(FM_buf3, DDR3buff+DDR_OFFSET, 6 + c+1);
			convolution3x3(FM_buf1, FM_buf2, weight_buf_3x3[0], bias_buf[0], 0);
		}
		else {
			load_buf_from_DDR(FM_buf1, DDR3buff+DDR_OFFSET, 6 + c+1);
			convolution3x3(FM_buf3, FM_buf2, weight_buf_3x3[0], bias_buf[0], 0);
		}

		relu_copy_buf_to_DDR(DDR3buff+DDR_OFFSET, 12 + c, FM_buf2, 0, 0);
	}
	...
	for(int co = 0; co < CO_N; co++) {
		...
		load_buf_from_DDR(FM_buf1, DDR3buff+DDR_OFFSET, 12 + 0);
		for(int ci = 0; ci < CI_N; ci++) {
			load_weight_1x1_from_axi(weight_buf_1x1[0], wright1x1[weight_1x1_index + ci + co * CI_N]);

			if( ci % 2 == 0) {
				load_buf_from_DDR(FM_buf2, DDR3buff+DDR_OFFSET, 12 + ci+1);
				CONV_1x1_bias(FM_buf1, FM_buf_acc, weight_buf_1x1[0],bias_buf[0],0,ci==0);
			}
			else {
				load_buf_from_DDR(FM_buf1, DDR3buff+DDR_OFFSET, 12 + ci+1);
		...
		/// DW5_CONV_3x3
		load_weight_3x3_from_axi(weight_buf_3x3[0], wright3x3[weight_3x3_index + co]);
		load_bias_from_axi(bias_buf[0], bias_all[bias_3x3_index + co]);
		Relu_Convert_FIX(FM_buf_acc, FM_buf3);
		convolution3x3(FM_buf3, FM_buf2, weight_buf_3x3[0], bias_buf[0], 0);
		relu_copy_buf_to_DDR(DDR3buff+DDR_OFFSET, 30 + co, FM_buf2, 0, 0);
...
//	bias_3x3_index += CI_N + CO_N;
	...
	for(int co = 0; co < CO_N; co++) {
		...
		load_bias_from_axi(bias_buf[0], bias_all[bias_1x1_index + co]);
		load_buf_from_DDR(FM_buf2, DDR3buff+DDR_OFFSET, 30 + 0);
		for(int ci = 0; ci < CI_N; ci++) {
			load_weight_1x1_from_axi(weight_buf_1x1[0], wright1x1[weight_1x1_index + ci + co * CI_N]);

			if( ci % 2 == 0) {
				load_buf_from_DDR(FM_buf1, DDR3buff+DDR_OFFSET, 30 + ci+1);
				CONV_1x1_bias(FM_buf2, FM_buf_acc, weight_buf_1x1[0],bias_buf[0],0,ci==0);
			}
			else {
				load_buf_from_DDR(FM_buf2, DDR3buff+DDR_OFFSET, 30 + ci+1);
				CONV_1x1_bias(FM_buf1, FM_buf_acc, weight_buf_1x1[0],bias_buf[0],0,ci==0);
			}
		}


		relu_copy_buf_to_DDR_acc(DDR3buff+DDR_OFFSET, 42 + co, FM_buf_acc, 0, 0);
		...
		//  CI_N = 512 / 32;
		load_weight_3x3_from_axi(weight_buf_3x3[0], wright3x3[weight_3x3_index + co + 24]);
		load_bias_from_axi(bias_buf[0], bias_all[bias_3x3_index + co + 24]);
		Relu_Convert_FIX(FM_buf_acc, FM_buf3);
		convolution3x3(FM_buf3, FM_buf2, weight_buf_3x3[0], bias_buf[0], 0);
		relu_copy_buf_to_DDR(DDR3buff+DDR_OFFSET, 82 + co, FM_buf2, 0, 0);
	}
...
//	bias_3x3_index += CI_N + CO_N;
	...
	for(int c = 0; c < CI_N; c+=4) {
		...
		load_and_reorg(DDR3buff+DDR_OFFSET, 100 + c/4,         DDR3buff+DDR_OFFSET, 100 + c/4 + 6,
					   DDR3buff+DDR_OFFSET, 100 + c/4 + 6 * 2, DDR3buff+DDR_OFFSET, 100 + c/4 + 6 * 3,
                       FM_buf1, FM_buf3, FM_buf4, FM_buf_acc);

		//// 1/4 channels
		load_weight_3x3_from_axi(weight_buf_3x3[0], wright3x3[weight_3x3_index + c + 0]);
		load_bias_from_axi(bias_buf[0], bias_all[bias_3x3_index + c + 0]);
		convolution3x3(FM_buf1, FM_buf2, weight_buf_3x3[0], bias_buf[0], 0);
		local_buf_copy(FM_buf1, FM_buf_acc);
		relu_copy_buf_to_DDR(DDR3buff+DDR_OFFSET, 58 + c + 0, FM_buf2, 0, 0);

		//// 2/4 channels
		load_weight_3x3_from_axi(weight_buf_3x3[1], wright3x3[weight_3x3_index + c + 1]);
		load_bias_from_axi(bias_buf[1], bias_all[bias_3x3_index + c + 1]);
		convolution3x3(FM_buf3, FM_buf2, weight_buf_3x3[1], bias_buf[1], 0);
		relu_copy_buf_to_DDR(DDR3buff+DDR_OFFSET, 58 + c + 1, FM_buf2, 0, 0);

		//// 3/4 channels
		load_weight_3x3_from_axi(weight_buf_3x3[2], wright3x3[weight_3x3_index + c + 2]);
		load_bias_from_axi(bias_buf[0], bias_all[bias_3x3_index + c + 2]);
		convolution3x3(FM_buf4, FM_buf2, weight_buf_3x3[2], bias_buf[0], 0);
		relu_copy_buf_to_DDR(DDR3buff+DDR_OFFSET, 58 + c + 2, FM_buf2, 0, 0);

		//// 4/4 channels
		load_weight_3x3_from_axi(weight_buf_3x3[3], wright3x3[weight_3x3_index + c + 3]);
		load_bias_from_axi(bias_buf[1], bias_all[bias_3x3_index + c + 3]);
		convolution3x3(FM_buf1, FM_buf2, weight_buf_3x3[3], bias_buf[1], 0);
		relu_copy_buf_to_DDR(DDR3buff+DDR_OFFSET, 58 + c + 3, FM_buf2, 0, 0);
	}
	...
	for(int co = 0; co < CO_N; co++) {
		...
		load_buf_from_DDR(FM_buf2, DDR3buff+DDR_OFFSET, 58 + 0);
		for(int ci = 0; ci < CI_N; ci++) {
			load_weight_1x1_from_axi(weight_buf_1x1[0], wright1x1[weight_1x1_index + ci + co * CI_N]);

			if( ci % 2 == 0) {
				load_buf_from_DDR(FM_buf1, DDR3buff+DDR_OFFSET, 58 + ci+1);
				CONV_1x1_bias(FM_buf2, FM_buf_acc, weight_buf_1x1[0],bias_buf[0],0,ci==0);
			}
			else {
				load_buf_from_DDR(FM_buf2, DDR3buff+DDR_OFFSET, 58 + ci+1);
				CONV_1x1_bias(FM_buf1, FM_buf_acc, weight_buf_1x1[0],bias_buf[0],0,ci==0);
			}
		}

		relu_copy_buf_to_DDR_acc(DDR3buff+DDR_OFFSET, 98 + co, FM_buf_acc, 0, 0);
	}
	...
	for(int co = 0; co < CO_N; co++) {
		...
		for(int ci = 0; ci < CI_N; ci++) {
			load_weight_1x1_from_axi(weight_buf_1x1[0], wright1x1[weight_1x1_index + ci + co * CI_N]);
			load_buf_from_DDR(FM_buf2, DDR3buff+DDR_OFFSET, 98 + ci);
			CONV_1x1_bias(FM_buf2, FM_buf_acc, weight_buf_1x1[0],bias_buf[0],0,ci==0);
		}
	}
